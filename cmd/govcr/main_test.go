package main

import (
	"testing"

	"github.com/stretchr/testify/require"
)

func TestMain_decryptCommand_EncryptionV1(t *testing.T) {
	got, err := decryptCassette("./test-fixtures/TestExample4.cassette.enc_v1.json", "./test-fixtures/TestExample4.unsafe.key")
	require.NoError(t, err)

	expected := `{
  "Tracks": [
    {
      "Request": {
        "Method": "GET",
        "URL": {
          "Scheme": "http",
          "Opaque": "",
          "User": null,
          "Host": "example.com",
          "Path": "/foo",
          "RawPath": "",
          "ForceQuery": false,
          "RawQuery": "",
          "Fragment": "",
          "RawFragment": ""
        },
        "Proto": "HTTP/1.1",
        "ProtoMajor": 1,
        "ProtoMinor": 1,
        "Header": {},
        "Body": "",
        "ContentLength": 0,
        "TransferEncoding": [],
        "Close": false,
        "Host": "example.com",
        "Form": null,
        "PostForm": null,
        "MultipartForm": null,
        "Trailer": null,
        "RemoteAddr": "",
        "RequestURI": ""
      },
      "Response": {
        "Status": "404 Not Found",
        "StatusCode": 404,
        "Proto": "HTTP/1.1",
        "ProtoMajor": 1,
        "ProtoMinor": 1,
        "Header": {
          "Accept-Ranges": [
            "bytes"
          ],
          "Age": [
            "527190"
          ],
          "Cache-Control": [
            "max-age=604800"
          ],
          "Content-Type": [
            "text/html; charset=UTF-8"
          ],
          "Date": [
            "Sun, 14 Aug 2022 18:12:40 GMT"
          ],
          "Expires": [
            "Sun, 21 Aug 2022 18:12:40 GMT"
          ],
          "Last-Modified": [
            "Mon, 08 Aug 2022 15:46:10 GMT"
          ],
          "Server": [
            "ECS (bsa/EB1E)"
          ],
          "Vary": [
            "Accept-Encoding"
          ],
          "X-Cache": [
            "404-HIT"
          ]
        },
        "Body": "",
        "ContentLength": -1,
        "TransferEncoding": [],
        "Trailer": null,
        "TLS": null,
        "Request": null
      },
      "ErrType": null,
      "ErrMsg": null,
      "UUID": "bf044888-81fd-43c0-a9cc-996accd9bf37"
    }
  ]
}`

	require.Equal(t, expected, got)
}

func TestMain_decryptCommand_EncryptionV2(t *testing.T) {
	got, err := decryptCassette("./test-fixtures/TestExample4.cassette.enc_v2.json", "./test-fixtures/TestExample4.unsafe.key")
	require.NoError(t, err)

	expected := `{
  "Tracks": [
    {
      "Request": {
        "Method": "GET",
        "URL": {
          "Scheme": "http",
          "Opaque": "",
          "User": null,
          "Host": "example.com",
          "Path": "/foo",
          "RawPath": "",
          "ForceQuery": false,
          "RawQuery": "",
          "Fragment": "",
          "RawFragment": ""
        },
        "Proto": "HTTP/1.1",
        "ProtoMajor": 1,
        "ProtoMinor": 1,
        "Header": {},
        "Body": "",
        "ContentLength": 0,
        "TransferEncoding": [],
        "Close": false,
        "Host": "example.com",
        "Form": null,
        "PostForm": null,
        "MultipartForm": null,
        "Trailer": null,
        "RemoteAddr": "",
        "RequestURI": ""
      },
      "Response": {
        "Status": "404 Not Found",
        "StatusCode": 404,
        "Proto": "HTTP/1.1",
        "ProtoMajor": 1,
        "ProtoMinor": 1,
        "Header": {
          "Accept-Ranges": [
            "bytes"
          ],
          "Age": [
            "194662"
          ],
          "Cache-Control": [
            "max-age=604800"
          ],
          "Content-Type": [
            "text/html; charset=UTF-8"
          ],
          "Date": [
            "Wed, 17 Aug 2022 21:50:32 GMT"
          ],
          "Expires": [
            "Wed, 24 Aug 2022 21:50:32 GMT"
          ],
          "Last-Modified": [
            "Mon, 15 Aug 2022 15:46:10 GMT"
          ],
          "Server": [
            "ECS (bsa/EB1E)"
          ],
          "Vary": [
            "Accept-Encoding"
          ],
          "X-Cache": [
            "404-HIT"
          ]
        },
        "Body": "",
        "ContentLength": -1,
        "TransferEncoding": [],
        "Trailer": null,
        "TLS": null,
        "Request": null
      },
      "ErrType": null,
      "ErrMsg": null,
      "UUID": "fb93c765-a370-430d-90af-b670c22f2b98"
    }
  ]
}`

	require.Equal(t, expected, got)
}
